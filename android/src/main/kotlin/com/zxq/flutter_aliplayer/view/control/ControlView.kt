package com.zxq.flutter_aliplayer.view.control
...
import com.zxq.flutter_aliplayer.view.quality.QualityItem
...
class ControlView : RelativeLayout, ViewAction, ITheme {
    ...
    //视频缓冲进度
    private var mVideoBufferPosition: Int = 0
    //这些是大小屏都有的==========END========
    ...
    private var mHideType: ViewAction.HideType? = null

    //saas,还是mts资源,清晰度的显示不一样
    private var isMtsSource: Boolean = false
...
//        mTitleBar = findViewById(R.id.titlebar)
        ...
        mTitlebarBackBtn = findViewById<View>(R.id.alivc_title_back) as ImageView
        mTitlebarText = findViewById<View>(R.id.alivc_title_title) as TextView
//        mTitleDownload = findViewById<View>(R.id.alivc_title_download) as ImageView
        mTitleMore = findViewById(R.id.alivc_title_more)
        mScreenModeBtn = findViewById<View>(R.id.alivc_screen_mode) as ImageView
        mScreenLockBtn = findViewById<View>(R.id.alivc_screen_lock) as ImageView
        mPlayStateBtn = findViewById<View>(R.id.alivc_player_state) as ImageView
        ...
        mLargeInfoBar = findViewById(R.id.alivc_info_large_bar)
        mLargePositionText = findViewById<View>(R.id.alivc_info_large_position) as TextView
        mLargeDurationText = findViewById<View>(R.id.alivc_info_large_duration) as TextView
        mLargeSeekbar = findViewById<View>(R.id.alivc_info_large_seekbar) as SeekBar
        mLargeChangeQualityBtn = findViewById<View>(R.id.alivc_info_large_rate_btn) as Button

        mSmallInfoBar = findViewById(R.id.alivc_info_small_bar)
        mSmallPositionText = findViewById<View>(R.id.alivc_info_small_position) as TextView
        mSmallDurationText = findViewById<View>(R.id.alivc_info_small_duration) as TextView
...
//        }
        ...
        val seekBarChangeListener = object : SeekBar.OnSeekBarChangeListener {
            override fun onProgressChanged(seekBar: SeekBar, progress: Int, fromUser: Boolean) {
                if (fromUser) {
                    ...
                    if (mAliyunScreenMode == AliyunScreenMode.Full) {
                        //全屏状态.
                        mLargePositionText!!.text = TimeFormater.formatMs(progress.toLong())
                    } else if (mAliyunScreenMode == AliyunScreenMode.Small) {
                        //小屏状态
                        mSmallPositionText!!.text = TimeFormater.formatMs(progress.toLong())
                    }
            ...
            override fun onStopTrackingTouch(seekBar: SeekBar) {
                ...
                mHideHandler.removeMessages(WHAT_HIDE)
                mHideHandler.sendEmptyMessageDelayed(WHAT_HIDE, DELAY_TIME.toLong())
            }
        }
        //seekbar的滑动监听
        ...
        mLargeChangeQualityBtn!!.setOnClickListener { v ->
            ...
            if (mOnQualityBtnClickListener != null && mAliyunMediaInfo != null) {
                val qualityTrackInfos = ArrayList<TrackInfo>()
                val trackInfos = mAliyunMediaInfo!!.trackInfos
...
//        updateChangeQualityBtn()
    ...
    private fun updateSeekBarTheme(theme: AliyunVodPlayerView.Theme) {
        ...
        val smallProgressDrawable = ContextCompat.getDrawable(context, progressDrawableResId)
        ...
        val largeProgressDrawable = ContextCompat.getDrawable(context, progressDrawableResId)
...
//        updateDownloadBtn()
    ...
    private fun updateChangeQualityBtn() {
        if (mLargeChangeQualityBtn != null) {
            ...
            mLargeChangeQualityBtn!!.text = QualityItem.getItem(context, mCurrentQuality!!, isMtsSource).name
    ...
    private fun updateSmallInfoBar() {
        if (mAliyunScreenMode == AliyunScreenMode.Full) {
            ...
            if (mAliyunMediaInfo != null) {
                mSmallDurationText!!.text = "/" + TimeFormater.formatMs(mAliyunMediaInfo!!.duration.toLong())
                mSmallSeekbar!!.max = mAliyunMediaInfo!!.duration
            } else {
                mSmallDurationText!!.text = "/" + TimeFormater.formatMs(0)
                mSmallSeekbar!!.max = 0
            }
            ...
            if (isSeekbarTouching) {
                ...
                mSmallPositionText!!.text = TimeFormater.formatMs(mVideoPosition.toLong())
            }
            //然后再显示出来。
    ...
    private fun updateLargeInfoBar() {
        if (mAliyunScreenMode == AliyunScreenMode.Small) {
            ...
            //先更新大屏的info数据
            if (mAliyunMediaInfo != null) {
                mLargeDurationText!!.text = "/" + TimeFormater.formatMs(mAliyunMediaInfo!!.duration.toLong())
                mLargeSeekbar!!.max = mAliyunMediaInfo!!.duration
            } else {
                mLargeDurationText!!.text = "/" + TimeFormater.formatMs(0)
                mLargeSeekbar!!.max = 0
            }
            ...
            if (isSeekbarTouching) {
                ...
                mLargePositionText!!.text = TimeFormater.formatMs(mVideoPosition.toLong())
            }
//            mLargeChangeQualityBtn!!.text = QualityItem.getItem(context, mCurrentQuality!!, isMtsSource).name
            //然后再显示出来。
    ...
    fun setHideType(hideType: ViewAction.HideType) {
     ...
     * 隐藏画面
     */
    override fun hide(hideType: ViewAction.HideType) {
        if (mHideType != ViewAction.HideType.End) {
            mHideType = hideType
        }
        visibility = View.GONE
    ...
    interface OnQualityBtnClickListener {
         ...
         * @param currentQuality 当前清晰度
         */
        fun onQualityBtnClick(v: View, qualities: List<TrackInfo>, currentQuality: String?)
    ...
    companion object {
        ...
        private val WHAT_HIDE = 0
        private val DELAY_TIME = 5 * 1000 //5秒后隐藏
    }
}
